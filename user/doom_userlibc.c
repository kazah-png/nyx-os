...
// v5.9.33 userspace backing. Adapted from the recovered kernel doom_utils.c: the
// pure-C parts (str*/mem*/vsnprintf/atoi/atof and the FILE cookie machinery) are
// reused as-is; only the kernel touchpoints (kmalloc/vfs_*/serial/exit) are retargeted
// at NyxOS userspace syscalls here. The doomgeneric shim headers (via fake_stdlib.h)
// supply the types, so no kernel.h is needed.
#define DSYS_EXIT  0
#define DSYS_WRITE 1
#define DSYS_OPEN  3
#define DSYS_READ  4
#define DSYS_CLOSE 5
#define DSYS_SBRK  7
#define DSYS_LSEEK 46
static long dsys(long n, long a, long b, long c) {
    long r; register long r10 asm("r10") = 0;
    __asm__ volatile("syscall" : "=a"(r) : "a"(n),"D"(a),"S"(b),"d"(c),"r"(r10) : "rcx","r11","memory");
    return r;
}
// Globals the engine references. The WAD blob is left null so fopen takes the file
// path (loading /mnt/doom1.wad); sound is stubbed for this first cut.
...
void *memcpy(void *dest, const void *src, size_t n) {
    void* _r = dest; __asm__ volatile("rep movsb":"+D"(dest),"+S"(src),"+c"(n)::"memory"); return _r;
    (void)0;
    return dest;
}
...
void *malloc(size_t size) {
    ...
    long p = dsys(DSYS_SBRK, (long)size, 0, 0);  /* sbrk returns old break = block base */
    return (p > 0) ? (void*)p : 0;
}
void free(void *ptr) { (void)ptr; }  /* bump allocator: no per-block reclaim */
...
void exit(int status) { dsys(DSYS_EXIT, status, 0, 0); while(1); }
...
FILE *fopen(const char *path, const char *mode) {
    ...
    if (doom_wad_data && doom_wad_size > 0) {
        if (strcmp(path, "doom1.wad") == 0 || strcmp(path, "/doom1.wad") == 0 || strcmp(path, "/boot/doom1.wad") == 0) {
            ...
            nyx_file_hdr* hdr = (nyx_file_hdr*)malloc(sizeof(nyx_file_hdr));
    ...
    int idx = alloc_cookie();
    if (idx < 0) return NULL;
    int fd = (int)dsys(DSYS_OPEN, (long)path, 0, 0);
    if (fd < 0 && path[0] != '/') {
        char altpath[128];
        int w = snprintf(altpath, sizeof(altpath), "/mnt/%s", path);
        if (w > 0 && w < (int)sizeof(altpath)) fd = (int)dsys(DSYS_OPEN, (long)altpath, 0, 0);
        if (fd < 0) {
    ...
    dsys(DSYS_CLOSE, fd, 0, 0);
    cookies[idx].data = _buf;
    cookies[idx].size = (uint32_t)(_got > 0 ? _got : 0);
    cookies[idx].pos = 0;
    cookies[idx].fd = -1;                          /* fully read into _buf */
    nyx_file_hdr* hdr = (nyx_file_hdr*)malloc(sizeof(nyx_file_hdr));
    if (!hdr) { cookies[idx].in_use = 0; return NULL; }
    hdr->magic = FILE_MAGIC;
    hdr->index = idx;
...
int fclose(FILE *stream) {
    ...
    if (!c) return EOF;
    if (c->fd >= 0) dsys(DSYS_CLOSE, c->fd, 0, 0);
    c->in_use = 0;
    free(stream);
    return 0;
}
...
int mkdir(const char *pathname) {
    (void)pathname; return 0;  /* userspace: savegame dir mkdir is a no-op */
}
...
/* --- extra libc the engine link needs (v5.9.33) --- */
char** environ = 0;
int vprintf(const char* fmt, va_list ap) {
    char b[1024];
    int n = vsnprintf(b, sizeof(b), fmt, ap);
    if (n > 0) dsys(DSYS_WRITE, 1, (long)b, n);
    return n;
}
...
int putchar(int c) { char ch = (char)c; dsys(DSYS_WRITE, 1, (long)&ch, 1); return c; }
int puts(const char* s) {
    long n = 0; while (s[n]) n++;
    dsys(DSYS_WRITE, 1, (long)s, n);
    char nl = '\n'; dsys(DSYS_WRITE, 1, (long)&nl, 1);
    return (int)n + 1;
}
