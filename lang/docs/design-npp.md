...
| M6.3e | ✅ **generic enums** ([`genum.npp`](../examples/genum.npp)): `enum Maybe<T>` + explicit uses `Maybe<i64>` (type positions and construction `Maybe<i64>.Some{...}` / `Maybe<i64>.None`) lower to a concrete `__g_Maybe_i64` (payload type slots substituted); `match` arms need no rewrite. Reuses the fn span-splice. Suite stage [10f] | ✅ genum.npp monomorphizes and the lowered N compiles identically under `ncc` and `ngen` |
| M6.3f | Construction-site **inference** for generic enums (`Result.Ok{v:3}` with no `<...>`, inferred from the enclosing return type) | pending |
