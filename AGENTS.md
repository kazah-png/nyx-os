...
- Network: DHCP/DNS/TCP/HTTP all work (v5.7.3–v5.7.4); loopback + ICMP echo replies
  + a real `ping` with RTT/loss stats **done** (v5.7.17). Possible follow-ups: TCP
  retransmit/large multi-segment responses, and DNS reliability under high latency
  (bump the poll budget). Note: QEMU slirp's gateway doesn't answer ICMP echo, so
  off-guest `ping` is best verified against loopback or a pcap dump
