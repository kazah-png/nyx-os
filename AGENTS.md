...
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  (v5.7.13). Remaining follow-ups:
  1. **IO/event waits + niceties.** An IO/event wait queue (keyboard, net) is
     trickier than sleep/child waits — the compositor must still wake periodically
     for the clock/mouse/net, so it can't purely block on input; a hybrid (block
     with a short timeout) could work. Also: a `nice`/`renice` to set job weights
     from the shell; mtdemo/spin still `hlt`-park after finishing rather than exiting.
