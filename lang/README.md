...
| M4 | N++ front-end: type checker, structs/enums/match, `Result`/`?` | ✅ folded into N itself — the static checker (v0.3–0.4), structs/impl/enums/`match` (v0.5–0.9) and `Result`/`?` (v0.10) shipped in `ncc` rather than a separate front-end; a distinct `.npp` dialect remains future work ([docs/design-npp.md](docs/design-npp.md)) |
