...
| M6.4b | Lambdas inside generic templates: a lifted `__c_N<T>` per template, instantiated with it | the M6.3g nested-use machinery carries it |
| M6.4c | Captures (design §6.2): an explicit capture struct + a first parameter, computed by a scope scan; `own` captures move and the struct inherits must-consume. The `win.on(Event.Click, fn(e) …)` handler sugar rides on it | an event-handler example over the nwin surface; escape/negative tests |
