...
<p align="center">
  <img src="https://img.shields.io/badge/N-v0.3-825AD2?style=flat" />
  &nbsp;
...
| Status | **v0.3 — working** (see below) | P1 started (static checks live in `ncc`) |
...
dependencies) implements N v0.3 — type inference (typed `:=` bindings with an
`i64` default, interpolation that inserts `str` values as text, enforced
`mut`), static checks (undeclared names, unknown callees, call arity and
argument types are compile errors with `file:line` diagnostics), strict-C99
output — and is verified three ways:
