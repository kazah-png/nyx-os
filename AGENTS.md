...
- **Userspace polish** (ring-3 works as of v5.5.0; exec reaps on exit + file
  ...
  - `exec` (`switch_to_user_process`) is still **blocking** — the shell waits for
    the child to exit. `spawn_user_path` (v5.7.7) runs a user ELF *non-blocking*
    under the scheduler, but there's no `wait()`/foreground-job story yet.
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  procs and reaps them on exit; `usertest` spawns two `spin.elf` copies that run
  preemptively alongside the GUI. Remaining follow-ups:
  1. **`wait()`/job control.** Give the shell a way to spawn a foreground job and
     block on it (blocked state + wakeup on child exit), so `exec` can be built on
     `spawn` + `wait` instead of the separate blocking longjmp path.
  2. **Blocked states + wait queues.** Threads still busy-spin instead of sleeping
     (mtdemo/spin `hlt`-park after finishing; `sleep()` busy-waits on `tick_count`).
     A real blocked state + timer/IO wait queues would free the CPU and let a
     priority/nice scheme keep the compositor responsive under load.
  3. **Heap/VFS re-entrancy.** `kmalloc`/`kfree` are now `preempt_disable`-guarded;
