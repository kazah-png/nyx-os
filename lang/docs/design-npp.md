...
| P4 | 🔨 **2 of 3**: `#[user]` pointer flavor (v0.12 — hard no-implicit-conversion boundary, `as` as the audited crossing; the canonical-half range *proof* stays an `n++` front-end item) · `pageflags` W^X bitset (v0.13 — values only from PROT_* constants and `|`, total compile-time W^X proof, verified with a live anonymous mmap); remaining: capabilities (ring/height declarations) | a kernel-module example checked at `ring0` |
