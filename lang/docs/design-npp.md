...
| M6.4c2 | Captures by value: a lambda naming enclosing locals gets an environment struct (the locals' types read the M6.3d way — literal inits, declared parameters — or refused), allocated and filled at the closure's birth; `own` captures move and the struct inherits must-consume; the `win.on(Event.Click, fn(e) …)` handler sugar rides on it | an event-handler example over the nwin surface; escape/negative tests |
| M6.4c3 | `Fn` inside generic templates: one struct per instantiation, the nested-use machinery again | gclosure-style example over `Fn(T) -> T` |
