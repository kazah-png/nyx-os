...
- Network: DHCP/DNS/TCP/HTTP all work (v5.7.3–v5.7.4); loopback + ICMP echo replies
  ...
  receive **done** (v5.7.18). Possible follow-ups: TCP send-side segmentation for
  large request bodies, a passive-open/listen path (in-guest TCP servers), and DNS
  reliability under high latency. Note: QEMU slirp's gateway doesn't answer ICMP
  echo and the host ISP may intercept some HTTP — verify against loopback or a pcap
  dump (`qemu -object filter-dump,id=f0,netdev=n0,file=x.pcap` + a pcap parser).
