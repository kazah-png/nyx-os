...
| P4 | 🔨 **started**: `#[user]` pointer flavor shipped (v0.12 — hard no-implicit-conversion boundary, `as` as the audited crossing, syscall params markable; the canonical-half range *proof* stays an `n++` front-end item); remaining: `PageFlags`, capabilities | a kernel-module example checked at `ring0` |
