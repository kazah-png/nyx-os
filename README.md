...
<p align="center">
  ...
  &nbsp;
  <img src="https://img.shields.io/badge/version-v6.4.21-825AD2?style=flat" />
  &nbsp;
...
- `xbm` — the NyxOS package manager: `xbm install <name>` compiles a package from its recipe with `cc` and installs it to `/mnt/bin` (from where it runs by bare name); `xbm remove` uninstalls it and `xbm list` shows the repository
