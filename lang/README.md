...
<p align="center">
  <img src="https://img.shields.io/badge/N-v0.20-825AD2?style=flat" />
  &nbsp;
...
| Status | **v0.20 — working** (see below) | **P1–P4 complete, P5 started** (`own` types complete: must-consume, branch-aware moves, `#[drop]` destructors; GUI bindings wait on kernel window syscalls) |
...
dependencies) implements N v0.20 — type inference (typed `:=` bindings with an
`i64` default, interpolation that inserts `str` values as text — with `:x`/`:X` hex format specs — enforced
...
└── examples/
    ...
    └── nparse.n         ← M5: a complete toy compiler — interned identifiers, functions, recursion
```
